...
class TestConfigResolutionSource:
    ...
    def test_sources_not_included_by_default(self):
        ...
        t = Tract.open()
        t.config._llm_state.default_config = LLMConfig(model="gpt-4o")
        result = t.config._resolve_llm_config("chat")
        assert "_resolution_sources" not in result

    def test_sugar_source(self):
        """Sugar params are tracked as source='sugar'."""
        t = Tract.open()
        result = t.config._resolve_llm_config("chat", model="gpt-4o", include_sources=True)
    ...
    def test_tract_default_source(self):
        ...
        t = Tract.open()
        t.config._llm_state.default_config = LLMConfig(model="gpt-4o", temperature=0.7)
        result = t.config._resolve_llm_config("chat", include_sources=True)
        sources = result["_resolution_sources"]
    ...
    def test_llm_config_source(self):
        ...
        result = t.config._resolve_llm_config("chat", llm_config=cfg, include_sources=True)
        sources = result["_resolution_sources"]
    ...
    def test_mixed_sources(self):
        ...
        t = Tract.open()
        t.config._llm_state.default_config = LLMConfig(temperature=0.5)
        t.config.configure_operations(chat=LLMConfig(top_p=0.9))
        result = t.config._resolve_llm_config(
