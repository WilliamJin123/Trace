...
class TestAutoConfig:
    def test_evaluate_with_mock_llm(self, tract_instance):
        """AutoConfig should return ConfigSuggestion list from LLM."""
        llm_response = json.dumps({
            "suggestions": [
                {
                    "key": "temperature",
                    "current_value": None,
                    "suggested_value": 0.2,
                    "reasoning": "Lower temperature for more focused output"
                }
            ]
        })
        mock = MockLLMClient([llm_response])
        ...
        ac = AutoConfig(
            name="test-config",
            objective="Optimize for focused code generation",
            config_keys=["temperature", "compile_strategy"],
        )
        suggestions = ac.evaluate(tract_instance)

        assert len(suggestions) == 1
        assert isinstance(suggestions[0], ConfigSuggestion)
        assert suggestions[0].key == "temperature"
        assert suggestions[0].suggested_value == 0.2
        assert "focused" in suggestions[0].reasoning.lower()

    def test_evaluate_no_llm_returns_empty(self):
        """Without an LLM client, evaluate returns empty list."""
        t = Tract.open()
        ac = AutoConfig(
            name="test", objective="optimize", config_keys=["temperature"]
        )
        suggestions = ac.evaluate(t)
        assert suggestions == []

    def test_evaluate_llm_failure_returns_empty(self, tract_instance):
        """LLM failure should return empty (fail-open)."""
        class FailClient:
            def chat(self, messages, **kwargs):
        ...
        tract_instance.configure_llm(FailClient())
    ...
    def test_evaluate_filters_unknown_keys(self, tract_instance):
        """Suggestions for keys not in config_keys should be filtered out."""
        llm_response = json.dumps({
            "suggestions": [
                {"key": "temperature", "suggested_value": 0.3, "reasoning": "ok"},
                {"key": "unknown_key", "suggested_value": "x", "reasoning": "bad"},
            ]
        })
        mock = MockLLMClient([llm_response])
        tract_instance.configure_llm(mock)
    ...
    def test_apply_suggestions(self, tract_instance):
        """apply() should configure the tract with suggested values."""
        suggestions = [
            ConfigSuggestion(
                key="temperature",
                current_value=None,
        ...
        applied = AutoConfig.apply(tract_instance, suggestions)
        assert applied == 1

        # Verify config was set
        val = tract_instance.get_config("temperature")
        assert val == 0.2

    def test_evaluate_unparseable_returns_empty(self, tract_instance):
        """Unparseable LLM response should return empty."""
...
class TestConfigSuggestion:
    ...
    def test_fields(self):
        cs = ConfigSuggestion(
            ...
            reasoning="Better for large contexts",
        )
        assert cs.key == "compile_strategy"
        assert cs.current_value == "full"
        assert cs.suggested_value == "adaptive"
        assert cs.reasoning == "Better for large contexts"
...
class TestTractRoutingIntegration:
    ...
    def test_auto_configure_no_llm(self):
        """auto_configure without LLM should return empty."""
        t = Tract.open()
        suggestions = t.auto_configure("optimize for speed")
        assert suggestions == []

    def test_auto_configure_with_llm(self):
        """auto_configure should delegate to AutoConfig."""
        t = Tract.open()
        llm_response = json.dumps({
            "suggestions": [
                {
    ...
    def test_aauto_configure_async(self):
        """Async auto_configure should work."""
        t = Tract.open()
        llm_response = json.dumps({
            "suggestions": [
                {
                    ...
                    "current_value": None,
                    "suggested_value": "adaptive",
                    "reasoning": "Better for mixed content"
                }
            ]
        })
        mock = MockLLMClient([llm_response])
        ...
        suggestions = asyncio.run(
            t.aauto_configure("optimize context", config_keys=["compile_strategy"])
        )
        assert len(suggestions) == 1
        assert suggestions[0].key == "compile_strategy"
