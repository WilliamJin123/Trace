...
class TestTokenSavings:
    def test_compact_fewer_schemas_than_full(self, tract):
        full_tools = tract.as_tools(profile="full")
        compact_tools = tract.as_tools(profile="compact")
        assert len(compact_tools) < len(full_tools)
        assert len(compact_tools) == 7

    def test_compact_smaller_json_than_full(self, tract):
        full_json = json.dumps(tract.as_tools(profile="full"))
        compact_json = json.dumps(tract.as_tools(profile="compact"))
...
class TestConsumptionPaths:
    def test_as_tools_openai(self, tract):
        tools = tract.as_tools(profile="compact", format="openai")
        assert len(tools) == 7
        for tool in tools:
    ...
    def test_as_tools_anthropic(self, tract):
        tools = tract.as_tools(profile="compact", format="anthropic")
        assert len(tools) == 7
        for tool in tools:
            assert "name" in tool
    ...
    def test_as_callable_tools(self, tract):
        callables = tract.as_callable_tools(profile="compact")
