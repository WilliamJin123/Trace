"""Tests for t.config.set() API, get_config(), well-known key validation, and enforcement.

Covers:
- t.config.set(model="gpt-4o") stores config in DAG
- t.config.get("model") returns "gpt-4o"
- t.config.get_all() returns all settings
...
class TestConfigureBasic:
    """t.config.set() stores settings, t.config.get() retrieves them."""

    def test_configure_model(self):
        """t.config.set(model='gpt-4o') stores model in DAG."""
        with Tract.open() as t:
            info = t.config.set(model="gpt-4o")
            assert info is not None
            assert info.content_type == "config"
            assert t.config.get("model") == "gpt-4o"

    def test_configure_multiple_keys(self):
        """t.config.set() stores multiple keys at once."""
        with Tract.open() as t:
            t.config.set(model="gpt-4o", temperature=0.7, max_tokens=1000)
            assert t.config.get("model") == "gpt-4o"
            assert t.config.get("temperature") == 0.7
            assert t.config.get("max_tokens") == 1000

    def test_configure_returns_commit_info(self):
        """t.config.set() returns a CommitInfo object."""
        with Tract.open() as t:
            info = t.config.set(model="gpt-4o")
    ...
    def test_configure_message_auto_generated(self):
        """t.config.set() generates a commit message mentioning keys."""
        with Tract.open() as t:
            info = t.config.set(model="gpt-4o")
    ...
    def test_get_config_with_default(self):
        ...
        with Tract.open() as t:
            assert t.config.get("nonexistent", default="fallback") == "fallback"
...
class TestGetAllConfigs:
    """t.config.get_all() returns all resolved settings."""
...
class TestConfigurePrecedence:
    ...
    def test_multiple_overrides(self):
        ...
        with Tract.open() as t:
            t.config.set(model="v1")
            t.config.set(model="v2")
            t.config.set(model="v3")
            assert t.config.get("model") == "v3"
    ...
    def test_auto_compress_threshold_rejects_string(self):
        ...
        with Tract.open() as t:
            with pytest.raises(ValueError, match="auto_compress_threshold.*expects"):
                t.config.set(auto_compress_threshold="high")
...
class TestUnknownKeys:
    ...
    def test_unknown_key_stored(self):
        ...
        with Tract.open() as t:
            t.config.set(my_custom_setting="custom_value")
            assert t.config.get("my_custom_setting") == "custom_value"

    def test_unknown_key_any_type(self):
        """Unknown keys accept any type."""
        with Tract.open() as t:
            t.config.set(custom_list=[1, 2, 3])
            assert t.config.get("custom_list") == [1, 2, 3]

    def test_unknown_key_in_get_all(self):
        """Unknown keys appear in get_all_configs()."""
        with Tract.open() as t:
            t.config.set(model="gpt-4o", custom="value")
            result = t.config.get_all()
...
class TestConfigureNoneUnset:
    ...
    def test_none_value_type_not_validated(self):
        ...
        with Tract.open() as t:
            ...
            t.config.set(model=None)  # should not raise ValueError
...
class TestMaxCommitTokensEnforcement:
    ...
    def test_commit_exceeding_limit_blocked(self):
        ...
        with Tract.open() as t:
            t.config.set(max_commit_tokens=5)
    ...
    def test_commit_within_limit_allowed(self):
        ...
        with Tract.open() as t:
            t.config.set(max_commit_tokens=10000)
            info = t.user("Hi")
            assert info is not None
    ...
    def test_max_commit_tokens_unset_removes_enforcement(self):
        ...
        with Tract.open() as t:
            ...
            # Unset the limit
            t.config.set(max_commit_tokens=None)
