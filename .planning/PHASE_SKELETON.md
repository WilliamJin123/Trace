...
- Implement core operations:
  - `tract init` — initialize a context trace
  - `tract commit` — commit a context snapshot (append/edit types)
  - `tract log` — view commit history with token counts
  - `tract status` — current HEAD, token budget usage
  - `tract diff` — compare two commits
  - `tract reset --soft` — move HEAD, keep content available
  - `tract reset --hard` — move HEAD, discard forward history
  - `tract checkout <commit>` — restore context to a specific checkpoint
...
- `tract branch <name>` — create a named branch from current HEAD
- `tract switch <name>` — switch active branch
- `tract merge <branch>` — merge branch into current (preserves history, adds merge commit)
- `tract rebase <branch>` — rebase current onto target (clean history, branch disappears)
...
- `tract compress <range>` — summarize a range of commits into a single commit
- `tract compress --respect-pins` — compress but preserve pinned commits verbatim
...
- `tract reorder` — rearrange commits (context is order-sensitive, unlike git content)
...
- **Spawn pointers**: a commit in a parent agent's history can reference the root of a child agent's Trace
  - `tract spawn <agent-name>` — create a subagent trace linked to current commit
...
- **Context collapse**: when a subagent completes, parent receives a collapse commit
  ...
  - `tract collapse <agent-name>` — generate collapse commit
  - `tract expand <collapse-commit>` — pull subagent commits inline for debugging
...
- **Garbage collection**
  ...
  - `tract gc` — run garbage collection per policy
...
- **Enhanced CLI**
  - `tract graph` — visualize the commit DAG (like `git log --graph` but for agent traces)
  - `tract audit <query>` — answer questions like "what did agent-3 know when agent-1 decided X?"
  - `tract blame <content>` — trace origin of specific context content
