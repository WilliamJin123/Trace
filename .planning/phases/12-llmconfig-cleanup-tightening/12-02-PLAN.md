...
must_haves:
  truths:
    ...
    - "compress() raises when explicit LLM params (model=, llm_config=) provided without LLM client"
...
Also store it on PendingCompression (line ~501-511):
```python
...
class TestFullGenerationConfigCapture:
    ...
    def test_response_model_authoritative(self):
        ...
        t.system("You are helpful")
        t.user("Hello")
        resp = t.generate(model="requested-model")
