...
with e.connect() as c:
    ...
    c.execute(text('CREATE TABLE commits (commit_hash VARCHAR(64) PRIMARY KEY, tract_id VARCHAR(64), parent_hash VARCHAR(64), branch_name VARCHAR(255), operation VARCHAR(10), priority VARCHAR(10), response_to VARCHAR(64), created_at DATETIME, blob_hash VARCHAR(64) REFERENCES blobs(blob_hash), generation_config_json TEXT)'))
