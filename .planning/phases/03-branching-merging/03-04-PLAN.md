...
must_haves:
  truths:
    ...
    - "Cherry-pick detects when an EDIT commit's edit_target target doesn't exist on the target branch"
...
2. **models/merge.py** -- Add issue types (append to existing file):
   ...
   class CherryPickIssue(BaseModel):
       ...
       missing_target: str | None = None  # The edit_target hash that doesn't exist on target
       description: str = ""
...
3. **operations/rebase.py** -- Rebase and cherry-pick operations:
   ...
   **`replay_commit(original_row, new_parent_hash, commit_engine, blob_repo, *, edit_target_remap=None) -> CommitInfo`:**
   - Load original content from blob_repo (get blob, parse JSON, validate_content).
   - If edit_target_remap is provided and original has edit_target, remap it.
   ...
   **`cherry_pick(commit_hash, tract_id, commit_repo, ref_repo, blob_repo, commit_engine, parent_repo=None, *, resolver=None) -> CherryPickResult`:**
   ...
   - Check for issues:
     - If EDIT commit: check if edit_target target exists in current branch's history (use get_all_ancestors). If not, create CherryPickIssue(issue_type="edit_target_missing").
   ...
   - For EDIT commits (with target on current branch): create new EDIT commit with same edit_target.
   ...
   **`rebase(tract_id, target_branch, commit_repo, ref_repo, parent_repo, blob_repo, commit_engine, annotation_repo, *, resolver=None) -> RebaseResult`:**
   ...
   - **Semantic safety check:** For each commit being replayed, check if reordering affects meaning:
     - If commit is an EDIT: check if edit_target target exists in the target branch history. If not, create RebaseWarning(warning_type="edit_target_missing").
   ...
   - **Pre-flight: block if branch contains merge commits.** Before replay, check if any commit in the replay range has entries in `commit_parents` (via `parent_repo.get_parents()`). If so, raise `RebaseError("Cannot rebase branch containing merge commits")`. Merge commits cannot be replayed as single-parent commits without losing the second parent's history.
   - **Replay commits (atomic via session rollback):**
     - Detach HEAD at target branch tip (or move branch pointer).
     - **Wrap the entire replay loop in try/except.** On ANY failure (safety check, commit error, etc.), call `session.rollback()` to atomically undo all partial replays, ref updates, and parent entries. Since replay goes through `CommitEngine.create_commit()` directly (not `Tract.commit()`), no intermediate `session.commit()` fires — all work stays in one uncommitted transaction until the facade commits after success.
     - NOTE: `CommitEngine.create_commit()` does NOT check detached HEAD — that guard is only in `Tract.commit()`. The replay path intentionally bypasses the facade to avoid this guard.
     ...
     - Only after full success does the Tract facade caller call `session.commit()`.
...
3. **tests/test_rebase.py** -- Comprehensive test suite (~250+ lines):
   ...
   **Cherry-pick tests:**
   ...
   - `test_cherry_pick_edit_with_target_on_branch`: Cherry-pick EDIT commit where edit_target target exists on current branch. Works fine.
   - `test_cherry_pick_edit_missing_target`: Cherry-pick EDIT commit where edit_target target does NOT exist on current branch. Raises CherryPickError (no resolver).
