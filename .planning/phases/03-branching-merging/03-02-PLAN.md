...
1. **llm/resolver.py** -- Built-in OpenAI-powered resolver:
   ...
   class OpenAIResolver:
       ...
       def _format_issue(self, issue: object) -> str:
           ...
           # Use pre-loaded content text fields (content_a_text, content_b_text)
           # rather than CommitInfo.message (which is the commit description, not content).
           # ConflictInfo pre-loads blob content during detect_conflicts().
