...
2. **models/merge.py** -- Merge data models (Pydantic):
   ...
   class ConflictInfo(BaseModel):
       ...
       target_hash: str | None = None  # The commit being conflicted over (response_to target)
...
3. **operations/merge.py** -- Core merge logic:
   ...
   **`detect_conflicts(branch_a_commits, branch_b_commits, annotation_repo, tract_id) -> list[ConflictInfo]`:**
   - Classify each commit by operation (APPEND or EDIT).
   - **Both EDIT same target:** If any EDIT in branch A has the same `response_to` as an EDIT in branch B, that's a `both_edit` conflict.
   ...
   - **EDIT + APPEND:** If either branch has ANY EDIT and the other branch has APPENDs, that's an `edit_plus_append` conflict (per CONTEXT.md: EDITs are high-stakes).
   ...
   **`create_merge_commit(commit_engine, parent_repo, content, parent_hashes, *, message=None, metadata=None, generation_config=None) -> CommitInfo`:**
   - Call `commit_engine.create_commit()` with content (first parent is already HEAD from engine).
   - BUT: The commit_engine auto-reads HEAD for parent_hash. For merge commits, we need to ensure the commit hash includes all parents. So:
     - After create_commit(), insert all parents into commit_parents table via `parent_repo.add_parents(commit_hash, parent_hashes)`.
     - The commit_hash should include extra_parents. This requires passing extra_parents to hashing. To do this cleanly: add an `extra_parents` parameter to `CommitEngine.create_commit()` that gets forwarded to `compute_commit_hash()`.
   - Actually, a cleaner approach: add a `create_merge_commit()` method to `CommitEngine` that:
     1. Creates the commit with the first parent as HEAD (normal flow).
     2. Passes `extra_parents=[parent_hashes[1]]` to `compute_commit_hash()`.
     3. Records all parents in commit_parents table.
   - UPDATE `src/tract/engine/commit.py`: Add `create_merge_commit(content, parent_hashes, *, message, metadata, generation_config) -> CommitInfo` method. This is similar to `create_commit()` but:
     - Uses `parent_hashes[0]` as `parent_hash` on CommitRow.
     - Passes `extra_parents=parent_hashes[1:]` to `compute_commit_hash()`.
     - Requires a `parent_repo` reference (add to CommitEngine __init__: `parent_repo: CommitParentRepository | None = None`).
     - After saving CommitRow, calls `parent_repo.add_parents(commit_hash, parent_hashes)`.
   - This requires adding `parent_repo` to CommitEngine constructor. Update Tract.open() and from_components() to pass it.
  </action>
  <verify>
...
1. **tract.py** -- Add merge facade methods:
   ...
   - `merge(source_branch: str, *, resolver=None, strategy="auto", no_ff=False, auto_commit=False, model=None, delete_branch=False) -> MergeResult`:
     ...
     - If delete_branch and merge succeeded: delete source branch ref.
