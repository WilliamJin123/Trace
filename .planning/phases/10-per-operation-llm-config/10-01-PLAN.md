...
must_haves:
  ...
  key_links:
    ...
    - from: "src/tract/tract.py (orchestrate)"
      ...
      via: "config resolution fills OrchestratorConfig defaults"
...
def configure_operations(self, **operation_configs: LLMOperationConfig) -> None:
    ...
    Valid operation names: "chat", "merge", "compress", "orchestrate", "auto_message".
...
merge_config = self._resolve_llm_config("merge", model=model)
...
Update the _resolve_llm_config call to include the new params:
```python
...
In `compress()` method (around line 1752-1778), after getting llm_client but before calling compress_range, resolve config:

Add before the compress_range call:
```python
# Resolve per-operation LLM config for compress
llm_kwargs = self._resolve_llm_config("compress") if llm_client is not None else {}
```
...
In `orchestrate()` method (around line 2419-2433), before creating the Orchestrator, resolve defaults from operation config and merge into the OrchestratorConfig:

BEFORE the existing `if config is not None or llm_callable is not None:` block, add:
```python
# Resolve per-operation config defaults for orchestrate
orch_resolved = self._resolve_llm_config("orchestrate")

# Merge into OrchestratorConfig (config fields take priority over operation defaults)
if orch_resolved and config is None:
...
elif orch_resolved and config is not None:
    # Only fill in None/default fields from operation config
    if config.model is None and "model" in orch_resolved:
        config.model = orch_resolved["model"]
    if config.temperature == 0.0 and "temperature" in orch_resolved:
        config.temperature = orch_resolved["temperature"]
```

**6. Create comprehensive tests in tests/test_operation_config.py:**
...
**compress integration tests (3 tests):**
...
- `test_compress_call_level_uses_operation_defaults` -- verify the resolved config flows through compress_range

**orchestrate integration tests (2 tests):**
...
For orchestrate tests: mock the orchestrator to verify config was applied (this is harder to test end-to-end; a unit test of _resolve_llm_config("orchestrate") plus verifying the config merging logic is sufficient).

Total: ~29 tests.
  </action>
  <verify>
Run: `python -m pytest tests/test_operation_config.py -v` -- all tests pass.
Run: `python -m pytest tests/ -x --timeout=120` -- full suite passes (952 existing + ~29 new = ~981 tests).
  </verify>
  <done>
All 4 active LLM operations (chat/generate, merge, compress, orchestrate) resolve config through _resolve_llm_config(). Tests verify: (1) call-level overrides operation-level overrides tract-level, (2) each operation uses its configured model/temperature/max_tokens, (3) generation_config reflects actual resolved model, (4) backward compatibility (no config = identical behavior), (5) extra_kwargs forwarding works.
  </done>
</task>
