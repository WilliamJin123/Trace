  ...
  <action>
    ...
    **tract.py** -- Extend `Tract.compile()`:
    ...
    3. After the existing compile logic produces a `result: CompiledContext`, add reordering block:
       ...
       if order is not None:
           ...
           # Attach warnings to result (as _reorder_warnings attribute for caller inspection)
           result._reorder_warnings = warnings  # type: ignore[attr-defined]
       ```
    ...
    **tests/test_reorder.py** -- Create test file:
    ...
    1. Basic reordering (~5 tests):
       ...
       - `test_compile_partial_order`: compile(order=[h3, h1]) puts h3 first, h1 second, h2 at end (not in order list)
    ...
    2. Safety checks (~5 tests):
       ...
       - `test_warnings_accessible`: result._reorder_warnings is a list of ReorderWarning objects
  ...
  <action>
    **operations/compression.py** -- Add GC functions:
    ...
    1. `_get_all_reachable(tract_id, ref_repo, commit_repo, parent_repo, *, branch=None) -> set[str]`:
       ...
       - Use `commit_repo.get_ancestors()` for each tip (returns full chain)
    ...
    **tests/test_gc.py** -- Create test file:
    ...
    For orphan tests: Use `t.reset("hard", commit_hash)` or branch manipulation to create unreachable commits. For archive tests: First compress(), then gc() -- the compressed source commits are archives.
