...
must_haves:
  truths:
    ...
    - "Structural safety checks warn when edit_target chain is broken by reordering"
  ...
  <action>
    **operations/compression.py** -- Add reordering safety check functions:
    ...
    1. `check_reorder_safety(order, commit_repo, blob_repo) -> list[ReorderWarning]`:
       ...
       - For each commit in order:
         a. If commit is an EDIT (operation == CommitOperation.EDIT) and its edit_target target appears LATER in the order list: emit ReorderWarning(warning_type="edit_before_target", commit_hash=commit.commit_hash, description=f"EDIT commit {commit.commit_hash[:8]} appears before its target {commit.edit_target[:8]}", severity="structural")
         b. If commit has edit_target and the edit_target commit is NOT in the order list at all: emit ReorderWarning(warning_type="response_chain_break", commit_hash=commit.commit_hash, description=f"Commit {commit.commit_hash[:8]} references {commit.edit_target[:8]} which is not in the reordered set", severity="structural")
    ...
    **tests/test_reorder.py** -- Create test file:
    ...
    2. Safety checks (~5 tests):
       ...
       - `test_safety_response_chain_break`: Create commit with edit_target=c1, reorder without c1 -> warning
