...
- **Known limitation:** PinPolicy only evaluates HEAD on each commit(). Commits made during batch() are not individually evaluated (policy eval is correctly skipped during batch). Users should call `retroactive_scan()` after batch operations to catch any missed InstructionContent/SessionContent commits. This is documented in PinPolicy's docstring.
- Add `to_config` / `from_config` for serialization

**builtin/branch.py** -- BranchPolicy:
- `name = "auto-branch"`, `priority = 300`, `trigger = "commit"`
- Constructor: `content_type_window: int = 5` (look at last N commits), `switch_threshold: int = 4` (minimum type switches to trigger), `ignore_transitions: set[tuple[str, str]] | None = None` (transitions to exclude from counting)
- Default `ignore_transitions`: `{("dialogue", "tool_io"), ("tool_io", "dialogue")}` — tool-use cycling is normal workflow, not a tangent signal
- `evaluate(self, tract)`:
  ...
  4. Count content_type transitions: iterate consecutive pairs, count where type changes AND the (from_type, to_type) pair is NOT in `ignore_transitions`
  5. If transitions >= `self._switch_threshold`:
     - Generate branch name: `f"tangent/{tract.current_branch}/{uuid.uuid4().hex[:8]}"`
...
- `evaluate(self, tract)`:
  ...
  3. Get recent commits in a single call: `commits = tract.log(limit=self._min_commits + 1)` — provides both last activity time and commit count
  4. If no commits, return None
  5. Calculate age from `commits[0].created_at`: `(now - commits[0].created_at).total_seconds() / 86400`
  6. Check staleness: if `len(commits) <= self._min_commits` AND `age_days >= self._stale_days`:
...
25. `test_rebase_policy_stale_branch_detected` -- Use `unittest.mock.patch` to mock `datetime.now(timezone.utc)` to return a time 10 days in the future relative to the commit timestamps. Create a branch with 2 commits (below min_commits=3 threshold), then evaluate with the mocked time. Verify returns PolicyAction with action_type="archive" and correct archive_name. This MUST test the actual staleness calculation (age_days >= stale_days AND low commit count), not just guard clauses.
...
Adjust CompressPolicy to accept `summary_content: str | None = None` in constructor. If provided, include `"content": summary_content` in the PolicyAction params. This enables testing without LLM. Default is None (requires LLM or collaborative review). Document this parameter in the docstring as "for testing use — provides a fixed summary to avoid LLM dependency in tests."
  </action>
  <verify>
...
<task type="auto">
  <name>Task 3: Auto-load policy config on Tract.open() for restart persistence</name>
  <files>
    src/tract/tract.py
    tests/test_policy_integration.py
  </files>
  <action>
**tract.py** -- In the `open()` classmethod, after creating `policy_repo` and setting `tract._policy_repo`, add auto-loading of persisted policy config:

```python
# Auto-load persisted policy config
saved_config = tract.load_policy_config()
if saved_config is not None:
    from tract.policy.builtin import CompressPolicy, PinPolicy, BranchPolicy, RebasePolicy
    policy_type_map = {
        "auto-compress": CompressPolicy,
        "auto-pin": PinPolicy,
        "auto-branch": BranchPolicy,
        "auto-rebase": RebasePolicy,
    }
    policies = []
    for entry in saved_config.get("policies", []):
        policy_cls = policy_type_map.get(entry.get("name"))
...
This ensures:
- Policies saved via `save_policy_config()` are automatically reinstantiated on `Tract.open()`
- Only built-in policies are auto-loaded (custom runtime policies remain non-persistent per CONTEXT.md)
- Disabled policies (enabled=False) are skipped
- Unknown policy names are silently skipped (forward compatibility)

**tests/test_policy_integration.py** -- Add restart persistence test:

- `test_policy_config_survives_restart_file_backed`:
  1. Open a file-backed tract (not :memory:)
  2. Configure CompressPolicy + PinPolicy
  3. Call save_policy_config() with their configs
  4. Close the tract
  5. Re-open the same file
  6. Verify tract._policy_evaluator is not None
  7. Verify both policies are registered with correct settings
  8. Verify the policies evaluate correctly on the reopened tract
  </action>
  <verify>
Run `python -m pytest tests/test_policy_integration.py::test_policy_config_survives_restart_file_backed -v` -- passes.
  </verify>
  <done>
Policies auto-load from _trace_meta on Tract.open(). The must-have "Policy config persists to _trace_meta and survives restart" is now fully met. File-backed restart test confirms end-to-end persistence.
  </done>
</task>
...
11. Policies auto-load from _trace_meta on Tract.open() -- config survives restart for file-backed tracts
</success_criteria>
