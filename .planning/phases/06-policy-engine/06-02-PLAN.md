...
- `_reconstruct_proposal_fn(self, proposal_row: PolicyProposalRow) -> Callable`:
  - Build a closure from the stored `action_type` and `action_params_json` that calls `_execute_action`
  - This enables proposals loaded from DB after restart to remain approvable
  - Pattern: `lambda p: self._execute_action_from_stored(p.action)` where the action is reconstructed from the row's action_type + params

- `get_pending_proposals(self) -> list[PolicyProposal]`:
  - If policy_repo, load pending PolicyProposalRows from DB
  - For each row, convert to PolicyProposal domain object
  - Reconstruct `_execute_fn` via `_reconstruct_proposal_fn()` so proposals remain approvable after restart
  - Else return []
...
2. In `from_components()`, add optional parameters:
   - `policy_repo: PolicyRepository | None = None` — for DI/testing injection
   - `policy_evaluator: PolicyEvaluator | None = None` — for DI/testing injection
   - Wire them to `tract._policy_repo` and `tract._policy_evaluator` respectively

3. In `open()`, after creating spawn_repo:
...
def register_policy(self, policy) -> None:
    ...
    If no evaluator is configured, auto-creates one with default settings
    (using the existing policy_repo from open()). Call configure_policies()
    first if you need custom on_proposal callback or cooldown_seconds.
    """
...
4. Modify `compile()` method: Add policy evaluation BEFORE the compilation logic. **Insert BEFORE the existing `current_head = self.head` assignment (line ~439 in current code).** The policy evaluation block includes its own `current_head = self.head` re-read, which replaces/supersedes the original assignment. If policies don't fire, the re-read still happens and sets the same variable the rest of compile() uses. This ensures compile() always operates on post-policy HEAD:
...
if (
    ...
    # NOTE: If a policy uses autonomous mode (e.g., autonomous CompressPolicy),
    # execution happens inline and HEAD may change. The re-read below handles
    # this. Default collaborative mode creates proposals (no HEAD mutation).
