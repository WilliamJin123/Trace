...
must_haves:
  ...
  artifacts:
    - path: "src/tract/orchestrator/__init__.py"
      ...
      exports: ["OrchestratorConfig", "AutonomyLevel", "OrchestratorState", "TriggerConfig", "OrchestratorProposal", "ProposalResponse", "StepResult", "OrchestratorResult", "auto_approve", "log_and_approve", "cli_prompt", "reject_all"]
...
- `ToolCall(frozen=True)`: id (str), name (str), arguments (dict). Re-use from toolkit if same, or define here and re-export in toolkit. Since Plan 01 and 02 are parallel (wave 1), define ToolCall in BOTH places identically. Plan 03 will reconcile.
...
**__init__.py** -- Re-export all public types from config.py, models.py, callbacks.py. Must include `reject_all` alongside the other callbacks.
