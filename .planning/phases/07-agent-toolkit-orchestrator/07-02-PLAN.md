...
must_haves:
  truths:
    ...
    - "Built-in callbacks auto_approve, log_and_approve, and cli_prompt work correctly"
  ...
  artifacts:
    - path: "src/tract/orchestrator/__init__.py"
      ...
      exports: ["OrchestratorConfig", "AutonomyLevel", "OrchestratorState", "TriggerConfig", "OrchestratorProposal", "ProposalResponse", "StepResult", "OrchestratorResult", "auto_approve", "log_and_approve", "cli_prompt"]
...
- `TriggerConfig(frozen=True)`: on_commit_count (int | None = None), on_token_threshold (float | None = None), on_compile (bool = False). Kept simple -- periodic scheduling deferred.
...
- `ToolCall(frozen=True)`: id (str), name (str), arguments (dict). Re-use from toolkit if same, or define here and re-export in toolkit. Since Plan 01 already defines ToolCall in toolkit/models.py, import from there. BUT since Plan 01 and 02 are parallel (wave 1), define ToolCall here as canonical source and have Plan 01 reference it. Actually -- to avoid coupling between parallel plans: define ToolCall in BOTH places identically. Plan 03 will reconcile. Simplest: define it here since orchestrator is the primary consumer. The toolkit plan will define its own copy. Plan 03 reconciles.

  Actually, the cleanest approach: define ToolCall in `src/tract/orchestrator/models.py` as the canonical location. Since Plan 01 creates a ToolCall in toolkit/models.py AND this plan creates one here, Plan 03 will import from one place and re-export. For now, just create the models -- no cross-package imports between toolkit and orchestrator.

- `OrchestratorProposal`: proposal_id (str), recommended_action (ToolCall), reasoning (str), alternatives (list[ToolCall], default_factory=list), context_summary (str = ""), decision (str = "pending"), modified_action (ToolCall | None = None). Use `@dataclass` (mutable -- decision changes).
- `ProposalResponse(frozen=True)`: decision (str -- "approved", "rejected", "modified"), modified_action (ToolCall | None = None), reason (str = "").
...
- `OrchestratorResult(frozen=True)`: steps (list[StepResult], default_factory=list), state (str = "idle"), assessment (str = ""), total_tool_calls (int = 0). Add a `@property succeeded` that returns all steps with success=True.
...
- `cli_prompt(proposal: OrchestratorProposal) -> ProposalResponse`: Interactive CLI prompt. Import Rich lazily (inside function body -- Rich is optional [cli] extra). Display proposal in a Rich Panel: action name, reasoning, arguments. Prompt user for [a]pprove / [r]eject. Return corresponding ProposalResponse. If Rich not available, fall back to plain input().
...
**__init__.py** -- Re-export all public types from config.py, models.py, callbacks.py.
...
python -c "from tract.orchestrator import OrchestratorConfig, AutonomyLevel, OrchestratorState, TriggerConfig, OrchestratorProposal, ProposalResponse, StepResult, OrchestratorResult, auto_approve, log_and_approve, reject_all; print('imports OK')"
  ...
  <done>
All orchestrator configuration and model types importable. AutonomyLevel has MANUAL/COLLABORATIVE/AUTONOMOUS. OrchestratorConfig has autonomy_ceiling, max_steps, profile, triggers. Built-in callbacks return correct ProposalResponse values.
  </done>
</task>
...
- `build_assessment_prompt(token_count: int, max_tokens: int, commit_count: int, branch_name: str, recent_commits: list[str], task_context: str | None = None) -> str`: Builds the user-side assessment prompt. Format:
  ```
...
10. `test_auto_approve_callback` -- Returns ProposalResponse(decision="approved")
11. `test_log_and_approve_callback` -- Returns ProposalResponse(decision="approved"), logs via caplog
12. `test_reject_all_callback` -- Returns ProposalResponse(decision="rejected")
...
python -m pytest tests/test_orchestrator_models.py -v
  ...
  <done>
All 16 tests pass. Orchestrator models, config, callbacks, and prompts work correctly. OrchestratorError added to exceptions. Assessment prompt builder formats context state properly.
  </done>
</task>
...
3. `python -c "from tract.orchestrator import OrchestratorConfig, AutonomyLevel, auto_approve; print(AutonomyLevel.COLLABORATIVE); print(auto_approve.__name__)"` -- smoke test
</verification>
...
3. Three built-in callbacks (auto_approve, log_and_approve, reject_all) return correct ProposalResponse
4. Assessment prompt builder formats context state properly
