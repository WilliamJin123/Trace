...
must_haves:
  ...
  artifacts:
    - path: "src/tract/toolkit/__init__.py"
      ...
      exports: ["ToolDefinition", "ToolProfile", "ToolConfig", "ToolExecutor", "ToolResult", "get_all_tools", "get_profile", "SELF_PROFILE", "SUPERVISOR_PROFILE", "FULL_PROFILE"]
...
- `ToolCall(frozen=True)`: id (str), name (str), arguments (dict).
...
- `FULL_PROFILE` (name="full"): All tools with default descriptions. No description overrides.
...
**__init__.py** -- Re-export: ToolDefinition, ToolConfig, ToolProfile, ToolResult, ToolCall, ToolExecutor, get_all_tools, get_profile, SELF_PROFILE, SUPERVISOR_PROFILE, FULL_PROFILE.
  </action>
  <verify>
...
- `ToolDefinition`, `ToolProfile`, `ToolConfig`, `ToolResult`, `ToolCall`, `ToolExecutor` to imports and `__all__`.
...
Tests should use real Tract instances opened with `Tract.open(db_path=..., ...)` using `tmp_path` fixture. Follow existing test patterns from `tests/test_tract.py`.
  </action>
  <verify>
