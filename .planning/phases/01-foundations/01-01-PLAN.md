...
<task type="auto">
  ...
  <action>
    **1. Project setup (pyproject.toml):**
    ...
    - IMPORTANT: The package import name is `trace` but note this shadows Python's stdlib `trace` module. Use `trace_context` as the distribution name on PyPI. The import path is `from trace.models.content import ...` etc.
