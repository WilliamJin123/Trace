...
Diff algorithm: use `difflib.SequenceMatcher` on the list of serialized messages to find matching blocks, then classify each position as added/removed/modified/unchanged. SequenceMatcher correctly handles insertions and deletions in the middle of the message list, not just at the end.
...
For generation_config_changes: compare the last generation_config in commit A's chain vs the last in commit B's chain. These represent "what config was active at each point." Find keys where values differ and store as `generation_config_changes: dict[str, tuple[Any, Any]]` mapping field_name -> (old_value, new_value).
  </action>
  <verify>
...
def log(
    ...
    *,
    ...
    infos = [self._commit_engine.row_to_info(row) for row in ancestors]
    return infos
```

Change default limit from 10 to 20 per CONTEXT.md decision.

NOTE: This requires adding `op_filter: CommitOperation | None = None` parameter to `CommitRepository.get_ancestors()` ABC method and its SQLite implementation. The SQL implementation should filter with `WHERE operation = :op` when op_filter is set, applied during the chain walk query.
...
**3. Add `Tract._compile_at()` helper and `Tract.diff()` method:**

```python
def _compile_at(self, commit_hash: str) -> CompiledContext:
    """Compile at a specific commit, using LRU cache if available."""
    # Check LRU cache first
    if commit_hash in self._compile_cache:
        snapshot = self._compile_cache[commit_hash]
        # Move to end (most recently used)
        self._compile_cache.move_to_end(commit_hash)
        return snapshot.to_compiled_context()
