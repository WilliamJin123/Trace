...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/tract/tract.py"
      provides: "Tract.reset(), Tract.checkout(), Tract.resolve_commit(), Tract.is_detached, Tract.current_branch"
      contains: "def reset"
...
- If ref_or_prefix == "-": raise TraceError("'-' shortcut is only supported by checkout(). Use tract.checkout('-') to return to previous position.")
...
- Both modes store ORIG_HEAD, update PREV_HEAD, and move HEAD to target via update_head(). Return the target_hash.
- Currently both modes are identical at the storage level. The distinction is semantic: soft = "I might come back", hard = "I'm discarding forward history." CLI requires --force for hard.
- Phase 4 GC will differentiate: hard reset will mark orphaned commits for aggressive garbage collection, soft reset will preserve them under normal retention policies.
...
def reset(self, target: str, *, mode: str = "soft") -> str:
    ...
    Args:
        ...
        mode: "soft" (preserves forward history under normal GC retention) or "hard" (marks orphaned commits for aggressive GC). Currently identical at storage level; behavior diverges when Phase 4 GC is implemented.
...
def is_detached(self) -> bool:
    """Whether HEAD is in detached state (pointing directly to a commit, not a branch)."""
