...
def _get_tract(ctx: click.Context) -> Tract:
    ...
    Auto-discovers tract_id if only one tract exists in the database.
    Errors if zero tracts exist. Requires --tract-id if multiple exist.
    """
    if "tract" not in ctx.obj:
        db_path = ctx.obj["db_path"]
        tract_id = ctx.obj.get("tract_id")
        if tract_id:
            t = Tract.open(db_path, tract_id=tract_id)
        else:
            t = _discover_tract(db_path)
        ctx.call_on_close(t.close)
        ctx.obj["tract"] = t
    return ctx.obj["tract"]


def _discover_tract(db_path: str) -> Tract:
    """Find the single existing tract in a database.

    Raises click.ClickException if zero or multiple tracts exist.
    """
    from sqlalchemy import create_engine, select, text
    from sqlalchemy.orm import Session
    engine = create_engine(f"sqlite:///{db_path}")
    with Session(engine) as session:
        # Query distinct tract_ids from commits table
        try:
            result = session.execute(
                text("SELECT DISTINCT tract_id FROM commits")
            ).scalars().all()
        except Exception:
            raise click.ClickException(
                f"No tract database found at '{db_path}'. "
                "Create one via the SDK first."
            )
    engine.dispose()
    if len(result) == 0:
        raise click.ClickException(
            f"No tracts found in '{db_path}'. Create one via the SDK first."
        )
    if len(result) > 1:
        ids = ", ".join(str(r) for r in result[:5])
        raise click.ClickException(
            f"Multiple tracts found in '{db_path}': {ids}. "
            "Use --tract-id to specify which one."
        )
    return Tract.open(db_path, tract_id=result[0])


@click.group()
@click.option("--db", default=".tract.db", envvar="TRACT_DB",
              help="Path to tract database.")
@click.option("--tract-id", default=None, envvar="TRACT_ID",
              help="Tract ID to operate on. Auto-discovered if only one tract exists.")
@click.pass_context
...
def checkout(ctx, target):
    ...
    try:
        ...
        if tract_obj.is_detached:
            ...
            console.print(f"Switched to branch '{tract_obj.current_branch}' at {resolved[:8]}")
    except TraceError as e:
        format_error(str(e), console)
        ctx.exit(1)
```
