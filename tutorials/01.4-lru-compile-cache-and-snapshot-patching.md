...
| Decision | Alternative | Why This Way |
|----------|-------------|--------------|
| Find target message by `commit_hashes.index()`, replace in-place, recount tokens | Full cache invalidation (Phase 1.1 behavior) | EDIT replaces one message. We know exactly which one (the commit it targets via `edit_target`). Rebuilding the entire snapshot from DB is wasteful when we can patch a single position. |
...
1. **Validate preconditions:** The snapshot must have `commit_hashes`, and the EDIT commit must have a `edit_target` target. If either is missing, return `None` to signal a full recompile fallback.
