...
a t.runtime.run() agent loop: assistant messages, tool calls, and tool results.

Usage:
    from _logging import StepLogger

    log = StepLogger()
    result = t.runtime.run("...", on_step=log.on_step, on_tool_result=log.on_tool_result)
"""
...
class StepLogger:
    ...
    def on_step(self, step_num: int, response: Any) -> None:
        """on_step callback for t.runtime.run().
    ...
    def on_tool_result(self, tool_name: str, output: str, status: str) -> None:
        """on_tool_result callback for t.runtime.run().
