...
Patterns shown:
  1. Creating Named Snapshots    -- t.persistence.snapshot("label") returns a tag
  2. Listing Snapshots           -- t.persistence.list_snapshots() for inspection
...
Demonstrates: t.persistence.snapshot(), t.persistence.list_snapshots(), t.persistence.restore_snapshot(),
              t.branches.create(), t.merge(), t.compile(), t.search.log()
...
def main() -> None:
    ...
    # =================================================================
    #
    # t.persistence.snapshot(label) creates a restore point at the current HEAD.
    ...
    with Tract.open() as t:
        ...
        tag = t.persistence.snapshot("after-initial-design")
        ...
        auto_tag = t.persistence.snapshot()
    ...
    # =================================================================
    #
    # t.persistence.list_snapshots() returns all snapshots as dicts with keys:
    ...
    with Tract.open() as t:
        ...
        t.persistence.snapshot("pre-migration-v2")
    ...
    with Tract.open() as t:
        ...
        restored_head = t.persistence.restore_snapshot("before-deep-audit")
    ...
    with Tract.open() as t:
        ...
        t.persistence.snapshot("pre-deploy-v3.2")
        ...
        commits_after = len(t.search.log(limit=50))
        print(f"  Commits after bad deploy: {commits_after}")

        # Direct reset -- no new branch, just rewind HEAD
        restored_head = t.persistence.restore_snapshot("pre-deploy-v3.2", create_branch=False)
    ...
    with Tract.open() as t:
        ...
        restored_head = t.persistence.restore_snapshot("before-experiment")
    ...
    with Tract.open() as t:
        ...
        t.branches.create("experiment/ai-features")
        ...
        t.persistence.snapshot("before-merge-ai-features")
        ...
        restored_head = t.persistence.restore_snapshot("before-merge-ai-features")
    ...
    print("  ----------------------------  -----------------------------------")
    print("  Create named snapshot         t.persistence.snapshot('label') -> tag string")
    print("  Create auto-labeled snapshot  t.persistence.snapshot() -> timestamp-based tag")
    print("  List all snapshots            t.persistence.list_snapshots() -> [dict, ...]")
    print("  Restore via branch (safe)     t.persistence.restore_snapshot('label')")
    print("  Restore via reset (fast)      t.persistence.restore_snapshot('x', create_branch=False)")
